...
Milestone 8 owner-approved scope — September 15, 2026
Health/Fervor coins are presentation-only. Extra earned Fervor adds flame-only tokens and never becomes Health; spent tokens dim. The display uses bonus-first spending solely to choose which visual token disappears. Default setup is one human plus two AI. Consented profile and telemetry storage, untracked play, Free-tier infrastructure, and local balance simulations are documented in MILESTONE-8-OPERATIONS.md. No balance or AI policy changed. The sole engine bug fix clears Cat Item movement state at Arena reset. Version fingerprints normalize line endings so Windows and Cloudflare identify the same rules consistently.
