...
import { trackController } from './analytics/browser';
...
function matchSeed(){return import.meta.env.DEV?12345:crypto.getRandomValues(new Uint32Array(1))[0];}
function App() {
  const [profile,setProfile]=useState<Profile|null|undefined>(),[profileOpen,setProfileOpen]=useState(false),[telemetryStatus,setTelemetryStatus]=useState('');
  const profileReady=profile!==undefined&&!profileOpen;
  const [controller,setController]=useState(()=>createController({seed:matchSeed(),playerCount:3,mode:'ai'}));
  const [,refresh]=useState(0);
  const [seed,setSeed]=useState('12345'),[count,setCount]=useState<2|3|4>(3),[error,setError]=useState('');
  ...
  const displacement=actions.some(a=>a.type==='SELECT_PUSHBACK'||a.type==='EFFECT_MOVE');
  const betActions=actions.filter(a=>a.type==='SELECT_BET');
  const overlayState=presentation.display?.state??(!displacement&&humanCombat(state,state.combat)?state:undefined);
  const boardActions=!profileReady?[]:overlayState||presentation.busy?[]:actions;
  useEffect(()=>{
    if(!profileReady||presentation.busy||betActions.length){setCountdown(undefined);return;}
  ...
  },[controller,revision,presentation.busy,betActions.length,profileReady]);
  useEffect(()=>{if(profileReady&&profile)return trackController(controller,profile.playerId,setTelemetryStatus);},[controller,profileReady,profile?.playerId]);
  const send=(action:GameAction)=>{if(!profileReady)return;try{controller.dispatch(action);setError('');refresh(n=>n+1);}catch(e){setError(String(e));}};
  const current=state.players[selected]??state.players.p1;
  const recent=state.eventLog.filter(e=>describeEvent(e)).slice(-5);
  useEffect(()=>{if(displacement&&!presentation.busy)document.querySelector('.arena-board')?.scrollIntoView({block:'center',behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'instant':'smooth'});},[displacement,presentation.busy]);
  ...
  return <main>{(profile===undefined||profileOpen)&&<ProfilePanel initial={profile} onDone={p=>{setProfile(p);setProfileOpen(false);}}/>}<Celebrations claims={celebrations.claims}/>{overlayState&&<CombatOverlay state={overlayState} actions={presentation.busy?[]:actions} send={send} rolling={presentation.display?.rolling??presentation.busy} result={presentation.display?.result} rollingDice={presentation.display?.rollingDice??presentation.pendingDice}/> }
    {betActions.length>0&&!presentation.busy&&<div className="modal-backdrop"><section className="modal" role="dialog" aria-modal="true" aria-label="Place your bet"><h2>{betActions[0].playerId.toUpperCase()}, place your bet</h2><p>Play is paused. Choose your predicted Arena winner for +1 Divine Favor if correct.</p>{betActions.map(a=>a.type==='SELECT_BET'&&<button key={a.targetId} onClick={()=>send(a)}>Bet on {a.targetId.toUpperCase()}</button>)}</section></div>}
    <header className="masthead"><div className="brand"><FavorIcon/><div><span className="eyebrow">THE GAMES OF THE</span><h1>Pigeon God</h1></div></div><nav><button onClick={()=>setProfileOpen(true)}>{profile?.name??'Player profile'}</button><button onClick={()=>setHelp(true)}>How to play</button><button onClick={()=>setSettings(true)}>Match settings</button><span className="mode-tag">{state.mode==='ai'?'Human vs AI':'Local multiplayer'}</span></nav></header>
    <div className="match-bar"><div><span className="eyebrow">{state.finalDuel?'SUDDEN DEATH':'THE ARENA AWAITS'}</span><h2>{state.finalDuel?'Final Duel':`Arena ${state.arenaNumber}`} <span>/ {state.finalDuel?'Last Rat standing':'2'}</span></h2></div><div className="rounds"><span>ROUND {state.roundNumber} / {state.finalDuel?'∞':'5'}</span><div>{[1,2,3,4,5].map(n=><i key={n} className={n<=state.roundNumber?'filled':''}/>)}</div></div><div className="turn-order"><span>TURN ORDER</span><p>{state.turnOrder.map(id=><b className={id===state.activePlayerId?'active':''} key={id}>{id.toUpperCase()}</b>)}</p></div><div className="cat-status"><img src={artUrl('cat.webp')} alt="Cat"/><span>THE CAT<br/><b>{state.cat.health} / 9 Health</b><small>4 Attack dice</small></span></div></div>
    {Object.values(state.players).filter(p=>p.eliminated&&p.controller==='human').map(p=><p className="elimination-notice" role="status" key={p.id}>{p.id.toUpperCase()} was eliminated. {p.eliminationRound!==undefined&&p.eliminationRound<=3&&!p.betTargetPlayerId?'Place your bet when prompted. ':''}On your Turn, choose a direction to move the Cat one hex. Earn 3 Divine Favor if that movement Finishes another Rat.</p>)}
