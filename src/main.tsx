...
function App() {
  const [controller,setController]=useState(()=>createController({seed:matchSeed(),playerCount:2}));
  ...
  useEffect(()=>{
    if(presentation.busy||betActions.length){setCountdown(undefined);return;}
  ...
  },[controller,revision,presentation.busy,betActions.length]);
  ...
  return <main><Celebrations claims={celebrations.claims}/>{overlayState&&<CombatOverlay state={overlayState} actions={presentation.busy?[]:actions} send={send} rolling={presentation.display?.rolling??presentation.busy} result={presentation.display?.result} rollingDice={presentation.display?.rollingDice??presentation.pendingDice}/> }
    ...
    <footer><span>THE GAMES OF THE PIGEON GOD</span>{import.meta.env.DEV&&<span>Milestone 7 · Playtest edition · Seed {seed}</span>}</footer>
