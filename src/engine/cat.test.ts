...
function catFight(s=fixture()):GameState {
  ...
  return dispatch(s,s.players[s.activePlayerId].eliminated?{type:'CHOOSE_CAT_DIRECTION',playerId:s.activePlayerId,direction:1}:{type:'ROLL_CAT_MOVEMENT',playerId:s.activePlayerId});
}
...
describe('Cat Turn lifecycle',()=>{
  ...
  it('eliminated player moves Cat exactly once and ends Turn',()=>{const s=fixture();s.players.p1.currentRat.alive=false;s.players.p1.currentRat.health=0;s.players.p1.eliminated=true;forceDirection(s,1);const rng=structuredClone(s.rng);const a=dispatch(s,{type:'CHOOSE_CAT_DIRECTION',playerId:'p1',direction:1});expect(a.rng).toEqual(rng);expect(a.activePlayerId).toBe('p2');expect(a.phase).toBe('CAT_MOVEMENT');expect(a.eventLog.filter(e=>e.type==='CAT_MOVED')).toHaveLength(1);expect(a.players.p1.actionsRemaining).toBe(0);});
});
describe('shared Cat combat',()=>{
  ...
  it('eliminated roller earns Favor for a Cat Finish',()=>{const f=fixture();f.players.p1.eliminated=true;f.players.p1.currentRat.alive=false;f.players.p1.currentRat.health=0;f.players.p2.currentRat.health=1;const s=resolveCat([4,4,4],[1,1,1],f);expect(s.players.p1.divineFavor).toBe(3);expect(s.players.p2.eliminated).toBe(true);expect(s.phase).toBe('ARENA_END');});
