...
export type GameEvent = {type:'DECREE_CLAIMED';playerId:string;cardId:string;name:string;reward:number;slot:number} | {type:'CONTENT';message:string} | {type:'BURROW_PLACED';playerId:PlayerId;position:HexCoordinate} | { type:'PHASE_CHANGED'; phase:GamePhase } | { type:'RAT_MOVED'; playerId:string; from:HexCoordinate; to:HexCoordinate; cost:number } | { type:'COMBAT_TRIGGERED'; attackerId:string; defenderId:string }
