...
function fixture():GameState {const s=createGame({seed:12345,playerCount:2});s.activePlayerId='p1';s.players.p1.actionsRemaining=2;return s;}
...
describe('setup and Turns (§§7–14,59)',()=>{
  it.each([2,3,4] as const)('creates %i seats and clockwise seeded order',playerCount=>{const s=createGame({seed:41,playerCount});expect(Object.keys(s.players)).toHaveLength(playerCount);expect(Object.values(s.players).filter(p=>p.controller==='human')).toHaveLength(1);expect(s).toEqual(createGame({seed:41,playerCount}));assertInvariants(s);});
  it('rejects invalid seeds',()=>expect(()=>createGame({seed:NaN,playerCount:2})).toThrow());
  it('provides two Actions at Turn start',()=>expect(fixture().players.p1.actionsRemaining).toBe(2));
  it('advances a Round only after all seats and stops at five',()=>{let s=fixture();for(let turn=0;turn<10;turn++){expect(s.roundNumber).toBe(Math.floor(turn/2)+1);s=dispatch(s,{type:'END_TURN',playerId:s.activePlayerId});}expect(s.phase).toBe('ARENA_END');expect(getLegalActions(s,s.activePlayerId)).toEqual([]);});
  it('retains eliminated seats with zero Actions',()=>{let s=fixture();s.players.p2.eliminated=true;s.players.p2.currentRat.alive=false;s.players.p2.currentRat.health=0;s=dispatch(s,{type:'END_TURN',playerId:'p1'});expect(s.activePlayerId).toBe('p2');expect(s.players.p2.actionsRemaining).toBe(0);expect(getLegalActions(s,'p2')).toEqual([{type:'END_TURN',playerId:'p2'}]);});
...
describe('movement (§§17–19,31,46–47,82)',()=>{
  ...
  it.each(['rat','cat'])('pauses at %s, keeps source and unused Speed, cannot cross',entity=>{const s=fixture();if(entity==='rat')s.players.p2.currentRat.position={q:-2,r:0};else s.cat.position={q:-2,r:0};const a=dispatch(s,{type:'MOVE',playerId:'p1',path:[{q:-2,r:0}]});expect(a.phase).toBe('COMBAT');expect(a.combat?.movementRemaining).toBe(2);expect(a.players.p1.currentRat.position).toEqual({q:-3,r:0});expect(()=>traceMovement(s,'p1',[{q:-2,r:0},{q:-2,r:-1}])).toThrow();expect(getLegalActions(a,'p1')).toEqual(entity==='cat'?[]:[{type:'CONFIRM_ATTACK',playerId:'p1'}]);});
  ...
  it('validates every generated action across 100 seeds and 2–4 seats',()=>{for(let seed=0;seed<100;seed++)for(const playerCount of [2,3,4] as const){let s=createGame({seed,playerCount});for(let i=0;i<6;i++){const legal=getLegalActions(s,s.activePlayerId);for(const action of legal)assertInvariants(simulate(s,action));const move=legal.find(a=>a.type==='MOVE' && !equal(a.path[a.path.length-1],s.cat.position));s=dispatch(s,move??legal[legal.length-1]);if(s.phase==='COMBAT')break;}}},30000);
