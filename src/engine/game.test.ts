...
function fixture():GameState {const s=createGame({automatic:false,mode:'ai',content:false,board:legacyBoard(),seed:12345,playerCount:2});s.phase='PLAYER_ACTION';s.activePlayerId='p1';s.players.p1.actionsRemaining=2;return s;}
...
describe('setup and Turns (§§7–14,59)',()=>{
  it.each([2,3,4] as const)('creates %i seats and clockwise seeded order',playerCount=>{const s=createGame({automatic:false,mode:'ai',content:false,board:legacyBoard(),seed:41,playerCount});expect(Object.keys(s.players)).toHaveLength(playerCount);expect(Object.values(s.players).filter(p=>p.controller==='human')).toHaveLength(1);expect(s).toEqual(createGame({automatic:false,mode:'ai',content:false,board:legacyBoard(),seed:41,playerCount}));assertInvariants(s);});
  it('rejects invalid seeds',()=>expect(()=>createGame({automatic:false,mode:'ai',content:false,board:legacyBoard(),seed:NaN,playerCount:2})).toThrow());
...
describe('movement (§§17–19,31,46–47,82)',()=>{
  ...
  it('validates every generated action across 100 seeds and 2–4 seats',()=>{for(let seed=0;seed<100;seed++)for(const playerCount of [2,3,4] as const){let s=createGame({automatic:false,mode:'ai',content:false,board:legacyBoard(),seed,playerCount});for(let i=0;i<6;i++){const legal=getLegalActions(s,s.activePlayerId);for(const action of legal)assertInvariants(simulate(s,action));const move=legal.find(a=>a.type==='MOVE' && !equal(a.path[a.path.length-1],s.cat.position));s=dispatch(s,move??legal[legal.length-1]);if(s.phase==='COMBAT')break;}}},30000);
