...
describe('Decrees and Arena lifecycle',()=>{
 ...
 it('Round-five Decrees do not award early and Cat Finishes meet generic thresholds',()=>{const s=fixture();s.content!.decrees=['herido','remata-uno'];s.players.p1.finishes=1;s.players.p1.currentRat.health=1;claimDecrees(s,'immediate');finishArena(s);expect(s.content!.claims.map(c=>c.cardId)).toEqual(['remata-uno']);});
