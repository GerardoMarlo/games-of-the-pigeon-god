...
describe('death and Finish (§§26,32–33)',()=>{
  ...
  it('dead attacker cannot continue and survivor keeps position',()=>{const f=fixture();f.players.p1.currentRat.health=1;const s=resolve([1,1,1],[6,1,1],f);expect(s.movement).toBeUndefined();expect(s.players.p2.finishes).toBe(1);expect(getLegalActions(s,'p1')).toEqual([{type:'END_TURN',playerId:'p1'}]);});
});
describe('pushback and movement continuation (§§30–31)',()=>{
  it('winner selects a legal hex and occupies the contested hex',()=>{let s=resolve([4,4,4],[1,1,1]);expect(s.combat?.stage).toBe('PUSHBACK');expect(getLegalActions(s,'p2')).toEqual([]);const action=getLegalActions(s,'p1')[0];expect(action.type).toBe('SELECT_PUSHBACK');s=dispatch(s,action);expect(s.players.p1.currentRat.position).toEqual({q:-2,r:-1});expect(s.movement?.remaining).toBe(2);expect(s.players.p1.actionsRemaining).toBe(1);assertInvariants(s);});
  ...
  it('winner retreats when defender has no legal empty neighbor',()=>{const f=fixture();for(const h of neighbors(f.players.p2.currentRat.position)){const tile=f.board.hexes[key(h)];if(tile&&key(h)!==key(f.players.p1.currentRat.position))tile.terrain='rock';}const s=resolve([4,4,4],[1,1,1],f);expect(s.combat).toBeUndefined();expect(s.players.p1.currentRat.position).toEqual({q:-2,r:0});expect(s.players.p2.currentRat.position).toEqual({q:-2,r:-1});expect(s.movement?.remaining).toBe(2);});
  it('continues without spending an Action, including when none remain',()=>{const f=fixture();f.players.p1.actionsRemaining=1;let s=resolve([1,1,1],[1,1,1],f);expect(s.players.p1.actionsRemaining).toBe(0);const action=getLegalActions(s,'p1').find(a=>a.type==='CONTINUE_MOVE')!;expect(action).toBeDefined();s=dispatch(s,action);expect(s.players.p1.actionsRemaining).toBe(0);});
  it('repeated combat consumes remaining Speed, never another Action',()=>{let s=resolve([1,1,1],[1,1,1]);s=dispatch(s,{type:'CONTINUE_MOVE',playerId:'p1',path:[{q:-2,r:-1}]});expect(s.combat?.movementRemaining).toBe(1);expect(s.players.p1.actionsRemaining).toBe(1);});
  it('rejects fresh Move while continuation is pending and permits stop',()=>{let s=resolve([1,1,1],[1,1,1]);expect(()=>dispatch(s,{type:'MOVE',playerId:'p1',path:[{q:-3,r:0}]})).toThrow();s=dispatch(s,{type:'STOP_MOVEMENT',playerId:'p1'});expect(s.movement).toBeUndefined();expect(getLegalActions(s,'p1').some(a=>a.type==='MOVE')).toBe(true);expect(()=>dispatch(s,{type:'CONTINUE_MOVE',playerId:'p1',path:[{q:-3,r:0}]})).toThrow();});
  it('zero unused Speed has no continuation',()=>{const f=fixture();f.players.p1.draftedRats[0].speed=1;expect(resolve([1],[1],f).movement).toBeUndefined();});
  ...
  it('continuation enforces remaining Speed, even with a free Sewer exit',()=>{const f=fixture();let s=resolve([1,1,1],[1,1,1],f);s.movement!.remaining=1;expect(()=>dispatch(s,{type:'CONTINUE_MOVE',playerId:'p1',path:[{q:-3,r:0},{q:-3,r:1}]})).toThrow();s=dispatch(s,{type:'CONTINUE_MOVE',playerId:'p1',path:[{q:-2,r:1},{q:2,r:-1},{q:2,r:0}]});expect(s.players.p1.currentRat.position).toEqual({q:2,r:0});expect(s.players.p1.actionsRemaining).toBe(1);expect(s.movement).toBeUndefined();});
